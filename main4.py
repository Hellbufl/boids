...
class Boids:
    ...
    def draw(self):
        ...
        for i in range(O.n):
            oposition = O.obstacle_positions[i]
            ovelocity = O.obstacle_velocities[i]
            direction = ovelocity / np.linalg.norm(ovelocity)

            point_a = oposition + direction * size
            point_b = oposition + np.array([-direction[1], direction[0]]) * size / 4
            point_c = oposition - np.array([-direction[1], direction[0]]) * size / 4

            pg.draw.polygon(O.display, (130, 130, 255), (point_a, point_b, point_c))
