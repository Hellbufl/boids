...
class Boids:
    ...
    def __init__(self, n, s, r):
        ...
        self.space = QuadTree(int(np.log2(self.boundry_size.max() / r)) + 1, self.boundry_size)
        ...
        self.debug_space = QuadTree(int(np.log2(self.boundry_size.max() / r)) + 1, self.boundry_size)
        self.debug_rects = []
    ...
    def set_quad_tree(self):
        self.space.reset()
        self.space.insert_array(np.array(range(self.n[0])), self.boid_positions, np.zeros([self.n[0], 2]))
    ...
    def get_target_mask(self, current_pos, current_vel = np.zeros(2)):
        ...
        possible_boids = self.space.search(current_pos - self.radius, np.ones(2) * self.radius * 2)
        # boid_mask[possible_boids] = True

        for i in possible_boids:
            self.debug_rects.append(pg.Rect(self.boid_positions[i] * self.scale / 100, np.array([50, 50]) * self.scale / 100))
    ...
    def set_debug_rects(self, position):
        self.debug_position = self.to_screen_space(position)
        self.debug_rects = []

        boid_target_mask = self.get_target_mask(position)[0]
        boid_target_pos = self.boid_positions[boid_target_mask]
        
        self.debug_rects.append(pg.Rect((position - self.radius) * self.scale / 100, np.ones(2) * self.radius * self.scale / 50))
        # for i in range(boid_target_pos.shape[0]):
        #     self.debug_rects.append(pg.Rect(boid_target_pos[i] * self.scale / 100, np.array([50, 50]) * self.scale / 100))

        # min_distance = 0
        # for i in range(boid_target_pos.shape[0]):
        #     d = np.linalg.norm(boid_target_pos[i] - position)
        #     if d < np.linalg.norm(boid_target_pos[min_distance] - position):
        #         min_distance = i
        
        # if boid_target_pos.size > 0:
        #     self.debug_space.reset()
        #     target_pos = boid_target_pos[min_distance]
        #     areas = self.debug_space.insert(0, target_pos, np.zeros(2))
        #     for i in range(len(areas)):
        #         self.debug_rects.append(pg.Rect(areas[i][0] * self.scale / 100, areas[i][1] * self.scale / 100))
    ...
    def update(self):
        ...
        if not self.debug_mode or self.debug_mode == 5:
            ...
            self.set_debug_rects(self.to_world_space(pg.mouse.get_pos()))
        ...
        if self.debug_mode:
            ...
            if self.debug_mode == 10:
                for rect in self.debug_rects:
                    debug_surface = pg.Surface(self.boundry_size * self.scale / 100)
                    debug_surface.set_alpha(30)
                    pg.draw.rect(debug_surface, (255, 255 ,255), rect)
                    self.display.blit(debug_surface, self.to_screen_space(np.zeros(2)))
